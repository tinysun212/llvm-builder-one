...
echo START after_build
echo 7z a -so -ttar -r dummy *.obj *.lib *.inc *.tmp *.res *.manifest include bin/clang-tblgen.exe bin/llvm-tblgen.exe .ninja_deps .ninja_log PIPE 7z a -si -tgzip -bd -bso0 swift_llvm_cache.tar.gz
